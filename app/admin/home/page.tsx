...
import { HOME_SECTION_META } from "@/lib/home-types";
  ...
  return (
    <AdminShell user={session.user}>
      ...
      <p className="mb-6 text-[12px] leading-relaxed text-charcoal/60">
        首页 4 个区块：主产品展示、月光宝盒、客户证言、底部 CTA。点击进入编辑，
        产品 / 月光宝盒区块支持上传视频或粘贴外链。
      </p>
      ...
      {loading ? (
        ...
        <ul className="divide-y divide-ink-700/10 border border-ink-700/10 bg-white">
          {rows.map((r) => {
            const meta = HOME_SECTION_META[r.type as keyof typeof HOME_SECTION_META];
            return (
              <li
                key={r.id}
